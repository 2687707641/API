...
class Common extends Controller {
    ...
    public function image_edit($path, $type) {
        ...
        switch ($type) {
        case 'head_img':
            $image->thumb(200, 200, Image::THUMB_CENTER)->save(ROOT_PATH . 'public' . $path); // 覆盖保存
            break;
        }
    }

    public function check_cate_id($id) {
        $res = db('cate')->where('cate_id', $id)->find();
        if (!$res) {
            $this->return_msg(400, '该上级分类不存在!');
        }
    }

    /*    检查名字是否重复    */
    public function check_repeat($name, $type) {
        switch ($type) {
        case 'cate':
            $res = db('cate')->where('cate_name', $name)->find();
            if ($res) {
                $this->return_msg(400, '该分类已存在!');
            }
            break;

        case 'book':
            $res = db('book')->where('book_name', $name)->find();
            if ($res) {
